...
class MambaBlock(layers.Layer):
    ...
    def get_config(self):
        ...
        config.update({
            'model_input_dims': self.args.model_input_dims,
            'model_states': self.args.model_states,
            'projection_expand_factor': self.args.projection_expand_factor,
            'conv_kernel_size': self.args.conv_kernel_size,
            'delta_t_min': self.args.delta_t_min,
            'delta_t_max': self.args.delta_t_max,
            'delta_t_scale': self.args.delta_t_scale,
            'delta_t_init_floor': self.args.delta_t_init_floor,
            'conv_use_bias': self.args.conv_use_bias,
            'dense_use_bias': self.args.dense_use_bias,
    ...
    def from_config(cls, config):
        # Recréer ModelArgs à partir de la config
        modelargs = ModelArgs(
            model_input_dims=config.pop('model_input_dims'),
            model_states=config.pop('model_states'),
            projection_expand_factor=config.pop('projection_expand_factor'),
            conv_kernel_size=config.pop('conv_kernel_size'),
            delta_t_min=config.pop('delta_t_min'),
            delta_t_max=config.pop('delta_t_max'),
            delta_t_scale=config.pop('delta_t_scale'),
            delta_t_init_floor=config.pop('delta_t_init_floor'),
            conv_use_bias=config.pop('conv_use_bias'),
            dense_use_bias=config.pop('dense_use_bias'),
            dropout_rate=config.pop('dropout_rate'),
            vocab_size=1,  # Valeur temporaire, sera surchargée
...
class ResidualBlock(layers.Layer):
    ...
    def from_config(cls, config):
        modelargs = ModelArgs(
            ...
            dense_use_bias=config.pop('dense_use_bias'),
            dropout_rate=config.pop('dropout_rate'),
            vocab_size=1,  # Valeur temporaire
            loss='sparse_categorical_crossentropy',
        )
        return cls(modelargs, **config)
...
def init_model(args: ModelArgs):
    ...
    if args.use_lm_head:
        ...
        output_layer = layers.Dense(args.num_classes)(x)
    else:
        x = layers.Flatten()(x)
        x = layers.Dense(1024, activation=tf.nn.gelu)(x)
